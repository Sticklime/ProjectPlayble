...
export type HoverTransparencyOptions = {
    radius?: number;           // радиус «дырки» в метрах (по умолчанию 0.35)
    feather?: number;          // мягкость края [0..1], 0=жёсткий, 1=очень мягкий (по умолчанию 0.7)
    enabledOnStart?: boolean;  // включено с начала (по умолчанию true)
    maxBrushes?: number;       // максимум штампов (по умолчанию 256)
};
...
export class HoverTransparency {
    ...
    private targetMesh: THREE.Mesh; // первый найденный меш (для raycast)
    ...
    setFeather(f: number) {
        this.feather = THREE.MathUtils.clamp(f, 0, 1);
        this._updateUniforms();
    }
    ...
    addStampAt(worldPos: THREE.Vector3, radius = this.radius) {
        if (this.brushCount >= this.MAX_BRUSHES) return;
        this.brushes.push({ center: worldPos.clone(), radius: Math.max(0.001, radius) });
        const i = this.brushCount * 4;
        this.brushesBuffer[i + 0] = worldPos.x;
        this.brushesBuffer[i + 1] = worldPos.y;
        this.brushesBuffer[i + 2] = worldPos.z;
        this.brushesBuffer[i + 3] = Math.max(0.001, radius);
        this.brushCount++;
        this.brushTexDirty = true;
        this._updateUniforms();
    }
    ...
    updateFromPointer(camera: THREE.Camera, pointerClient: { x: number; y: number }, rendererDom: HTMLElement): THREE.Vector3 | undefined {
        ...
        const hit = hits.find(h => (h.object as any).isMesh);
        if (hit) {
            this.hoverCenter.copy(hit.point);
        } else {
            this.hoverCenter.set(99999, 99999, 99999);
        }
        this._updateUniforms();
    ...
    precompile(renderer: THREE.WebGLRenderer, camera: THREE.Camera) {
        try {
            renderer.compile(this.root, camera);
        } catch (e) {
            console.warn("[HoverTransparency] precompile failed:", e);
        }
    }
            ...
            if ((o as any).isMesh) {
                ...
                if (g && g.attributes && g.attributes['position']) {
                    ...
                    if (changed) {
                        ...
                        bad.push(m.name || m.uuid);
                    }
                }
            }
        });
        if (bad.length) {
            console.warn("[HoverTransparency] sanitized NaN positions for meshes:", bad);
        }
    }

    private _updateUniforms() {
        if (this.brushTexDirty) {
            if (this.brushTex.image) {
                this.brushTex.image.data = this.brushesBuffer;
            } else {
                this.brushTex.image = { data: this.brushesBuffer, width: this.brushTexSize, height: 1 } as any;
            }
...
float previewK = 0.0;
{
  float innerF = uHoverRadius * clamp(uHoverFeather, 0.0, 0.9999);
  float d = length(vWorldPos - uHoverCenter);
  float k = 1.0 - smoothstep(innerF, uHoverRadius, d);
  previewK = k;
}
float K = max(persistentK, previewK);
