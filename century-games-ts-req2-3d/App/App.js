...
let App = new Game({
	...
	prepare() {
		...
		if (window.MraidSDK) MraidSDK.on('Start Game', () => { this.startGame(); });
	...
	startGame() {
        ...
        if (Settings["video"] && App["Video"]) {
			...
			if (Settings["cta-only"] === true) {
				// В случае, если MraidSDK подключен и cta-only включено, MraidSDK покажет конечный экран самостоятельно
				if (!window.MraidSDK) App.CallToAction.show();
			} else {
				App.Gameplay.show();
			}
