...
export class TinyParticleEmitter extends Object3D {
  ...
  constructor(
    ...
    this.emitterOptions = {
      ...
      useRawTime: emitterOptions?.useRawTime ?? false,
    };
  ...
  public get positionRange(): TinyRangeVector3 {
    return this.particleOptions.positionRange;
  }

  public set positionRange(value: TinyRangeVector3) {
    this.particleOptions.positionRange = value;
  }

  public play(): void {
    if (!this.isPlaying) {
      this.isPlaying = true;
      const time = this.emitterOptions.useRawTime
        ? TimeController.instance.rawTime
        : TimeController.instance.time;
      this.lastTimeSpawned = time;
  ...
  private onTick(): void {
    if (this.isPlaying) {
      const time = this.emitterOptions.useRawTime
        ? TimeController.instance.rawTime
        : TimeController.instance.time;
      const elapsedTime = time - this.lastTimeSpawned;
      const spawnCount = Math.floor(
        this.emitterOptions.spawnRate * elapsedTime,
      );

      if (spawnCount > 0) {
        const realSpawnCount = Math.min(
          spawnCount,
          this.remainingParticleCount,
        );
        this.lastTimeSpawned = time;
  ...
  private spawnBillboard(): void {
    ...
    this.emitterOptions.system.spawnBillboard({
      ...
      scaleOverTime,
      opacityOverTime,
      localSpaceObject: this.emitterOptions.isLocalSpace ? this : undefined,
    });
  }
}
