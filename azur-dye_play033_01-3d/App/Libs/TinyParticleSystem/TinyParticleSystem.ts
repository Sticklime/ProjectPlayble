...
export class TinyParticleSystem {
  ...
  private readonly useRawDeltaTime: boolean;
  ...
  constructor(systemOptions: SystemOptions, materialOptions: MaterialOptions) {
    ...
    this.useRawDeltaTime = systemOptions.useRawDeltaTime ?? false;
  ...
  private onTick(): void {
    const adjustedDeltaTime = this.useRawDeltaTime
      ? TimeController.instance.rawDeltaTime
      : TimeController.instance.deltaTime;
    ...
    for (const particle of this.particles) {
      particle.lifeTime += particle.lifeTimeFactor * adjustedDeltaTime;
      ...
      particle.velocity.addScaledVector(this.gravity, adjustedDeltaTime);

      const localSpaceObject = particle.localSpaceObject;
      if (localSpaceObject) {
        ...
        particle.position.addScaledVector(particle.velocity, adjustedDeltaTime);
      }

      particle.rotation += particle.angularVelocity * adjustedDeltaTime;
