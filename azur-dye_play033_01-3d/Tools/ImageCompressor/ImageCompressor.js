...
class ImageCompressor {
  ...
  getImageConfig(imageName) {
    ...
    const mergedConfig = {
      ...
      formatOptions: {
        webp: { ...(defaultConfig.formatOptions?.webp || {}) },
        jpeg: { ...(defaultConfig.formatOptions?.jpeg || {}) },
        png: { ...(defaultConfig.formatOptions?.png || {}) },
      },
    };
    ...
    // Apply custom format-specific options
    if (customConfig.formatOptions) {
      if (customConfig.formatOptions.webp) {
        mergedConfig.formatOptions.webp = {
          ...mergedConfig.formatOptions.webp,
          ...customConfig.formatOptions.webp,
        };
      }
      if (customConfig.formatOptions.jpeg) {
        mergedConfig.formatOptions.jpeg = {
          ...mergedConfig.formatOptions.jpeg,
          ...customConfig.formatOptions.jpeg,
        };
      }
      if (customConfig.formatOptions.png) {
        mergedConfig.formatOptions.png = {
          ...mergedConfig.formatOptions.png,
          ...customConfig.formatOptions.png,
        };
      }
    }
   ...
   * @param {Object} formatOptions - Продвинутые настройки форматов
  ...
  applyCompressionTransform(
    ...
    switch (outputFormat) {
      case "jpeg": {
        const jpegOpts = formatOptions.jpeg || {};
        const options = {
          quality: jpegOpts.quality !== undefined ? jpegOpts.quality : quality,
          mozjpeg: jpegOpts.mozjpeg !== undefined ? jpegOpts.mozjpeg : true,
          progressive:
            jpegOpts.progressive !== undefined
              ? jpegOpts.progressive
              : quality < 90,
        };

        if (options.mozjpeg) {
          if (jpegOpts.optimizeCoding !== undefined) {
            options.optimizeCoding = jpegOpts.optimizeCoding;
          }
          if (jpegOpts.trellisQuantisation !== undefined) {
            options.trellisQuantisation = jpegOpts.trellisQuantisation;
          }
          if (jpegOpts.overshootDeringing !== undefined) {
            options.overshootDeringing = jpegOpts.overshootDeringing;
          }
        }

        if (jpegOpts.chromaSubsampling !== undefined) {
          options.chromaSubsampling = jpegOpts.chromaSubsampling;
        }
      ...
      case "png": {
        const pngOpts = formatOptions.png || {};
        const options = {
          compressionLevel:
            pngOpts.compressionLevel !== undefined
              ? pngOpts.compressionLevel
              : Math.max(1, Math.min(9, Math.round((100 - quality) / 12))),
          progressive:
            pngOpts.progressive !== undefined
              ? pngOpts.progressive
              : quality < 90,
        };

        if (pngOpts.palette !== undefined && pngOpts.palette) {
          options.palette = true;
          if (pngOpts.quality !== undefined) {
            options.quality = pngOpts.quality;
          }
          if (pngOpts.effort !== undefined) {
            options.effort = pngOpts.effort;
          }
          if (pngOpts.colors !== undefined) {
            options.colors = pngOpts.colors;
          }
          if (pngOpts.dither !== undefined) {
            options.dither = pngOpts.dither;
          }
        } else if (pngOpts.quality !== undefined) {
          options.quality = pngOpts.quality;
        }
      ...
      case "webp": {
        const webpOpts = formatOptions.webp || {};
        const options = {
          quality: webpOpts.quality !== undefined ? webpOpts.quality : quality,
          lossless:
            webpOpts.lossless !== undefined ? webpOpts.lossless : quality >= 95,
        };

        if (webpOpts.nearLossless !== undefined) {
          options.nearLossless = webpOpts.nearLossless;
        }
        if (webpOpts.smartSubsample !== undefined) {
          options.smartSubsample = webpOpts.smartSubsample;
        }
        if (webpOpts.effort !== undefined) {
          options.effort = webpOpts.effort;
        }
        if (hasAlpha && webpOpts.alphaQuality !== undefined) {
          options.alphaQuality = webpOpts.alphaQuality;
        }
        if (webpOpts.minSize !== undefined) {
          options.minSize = webpOpts.minSize;
        }
        if (webpOpts.mixed !== undefined) {
          options.mixed = webpOpts.mixed;
        }
