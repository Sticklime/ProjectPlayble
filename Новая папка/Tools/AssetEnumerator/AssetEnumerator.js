...
class AssetEnumerator {
  ...
  async update() {
    ...
    // Cache old assets before update
    const oldAssets = this.extractOldAssets(raw);

    // Collect all assets from project directories
    const { priority, rest } = this.collectAssets();
    const combined = { ...priority, ...rest };

    // Preserve custom fields from old assets
    this.preserveCustomFields(combined, oldAssets);
  ...
  /**
   * Extracts current Assets from settings file
   * @param {string} raw - Settings file content
   * @returns {Object} Current assets object
   */
  extractOldAssets(raw) {
    const errors = [];
    const root = parseTree(raw, errors, { allowTrailingComma: true });

    if (errors.length > 0) {
      console.warn("⚠️ Warning: Could not parse old assets, skipping cache");
      return {};
    }

    const node = findNodeAtLocation(root, ["Assets"]);
    if (!node) {
      console.warn("⚠️ Warning: Assets section not found, skipping cache");
      return {};
    }

    try {
      const assetsJson = raw.slice(node.offset, node.offset + node.length);
      return JSON.parse(assetsJson);
    } catch (e) {
      console.warn("⚠️ Warning: Could not parse Assets JSON, skipping cache");
      return {};
    }
  }

  /**
   * Preserves custom fields from old assets to new assets
   * @param {Object} newAssets - New assets object to update
   * @param {Object} oldAssets - Old assets object with custom fields
   */
  preserveCustomFields(newAssets, oldAssets) {
    for (const [key, newAsset] of Object.entries(newAssets)) {
      const oldAsset = oldAssets[key];
      if (!oldAsset) continue;

      // Copy all fields from old asset except 'type' and 'url'
      for (const [field, value] of Object.entries(oldAsset)) {
