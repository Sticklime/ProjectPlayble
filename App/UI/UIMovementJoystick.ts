...
export enum UIMovementJoystickEvent {
  MOVE = "move",
  RELEASE = "release",
}

export class UIMovementJoystick extends UIGraphics {
  ...
  private readonly tutorialJoystickCap = new UIImage(
    this.layer,
    AssetKeeper.T_MovementJoystickCap,
  );
  private readonly tutorialJoystickText = new UIImage(
    this.layer,
    AssetKeeper.T_DragToMove,
  );
  ...
  private readonly onJoystickUp = (
    ...
    if (this.pointerIdentifier === identifier) {
      ...
      this.pointerIdentifier = undefined;
      this.emit(UIMovementJoystickEvent.RELEASE);
