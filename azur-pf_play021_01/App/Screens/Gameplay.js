...
App.Gameplay = new Screen({
    ...
    Hooks: {
        ...
        build() {
            ...
            App.World.Renderer.setClearColor("#F3D282", 1);
            ...
            this["main_light"].intensity = Settings["main-light-intensity"]
        ...
        resize: function () {
            ...
            if (isLandscape) {
                this.foldHints = [
                    {foldName: "folds_2", position: new THREE.Vector3(-280, -150, 0)},
                    {foldName: "folds_6", position: new THREE.Vector3(340, -150, 0)},
                    {foldName: "folds_4", position: new THREE.Vector3(0, 100, 0)},
                    {foldName: "folds_8", position: new THREE.Vector3(0, -500, 0)}
                ];
            } else {
                this.foldHints = [
                    {foldName: "folds_2", position: new THREE.Vector3(-300, -200, 0)},
                    {foldName: "folds_6", position: new THREE.Vector3(320, -200, 0)},
                    {foldName: "folds_4", position: new THREE.Vector3(0, 100, 0)},
                    {foldName: "folds_8", position: new THREE.Vector3(0, -450, 0)}
                ];
            }
            ...
            if (isLandscape) {
                ...
                this.paperInst.model.position.set(0, 1, 0.5)
            } else {
                this.paperInst.model.position.set(0, -2.5, 0.5)
            }
            ...
            const is4by3 = ratio > 0.9 && ratio < 1.1;
...
// Горизонтальный режим:
    ...
    updateHandHint() {
        const doneFolds = this.paperInst.getFoldPaper();
    ...
    animatePulse(obj, min = 0.96, max = 1.05, duration = 0.9) {
        let growing = true;
    ...
    createOrigamiModel(parent) {
        ...
        textureFront.anisotropy = maxAniso;
        ...
        model.rotation.set(Math.PI / 1.9, 0, 0);
        ...
        model.traverse(x => {
            ...
            if (x.isMesh) {
                ...
                if (x.name.endsWith('1')) {
                    x.material = new THREE.MeshStandardMaterial({...commonMat, map: textureFront});
                }

                if (x.name.endsWith('2')) {
                    x.material = new THREE.MeshStandardMaterial({...commonMat, map: textureBack});
                }

                if (x.name.startsWith('folds_8')) {
                    if (x.name.endsWith('2')) {
                        x.material = new THREE.MeshStandardMaterial({...commonMat, map: textureFront});
                    }
                    if (x.name.endsWith('1')) {
                ...
                if (x.name.startsWith('folds_2_1')) {
                    const mats = Array.isArray(x.material) ? x.material : [x.material];

                    mats.forEach((mat, i) => {
                        if (!mat || !mat.map) return;

                        if (!mat.userData?.__localMap) {
                            const cloned = mat.clone();
                            cloned.map = mat.map.clone();
                            cloned.userData = {...(cloned.userData || {}), __localMap: true};
                            if (Array.isArray(x.material)) x.material[i] = cloned;
                            else x.material = cloned;
                        ...
                        mat.map.offset.y += 0.0101;
                        mat.map.offset.x += 0.01;
                        mat.map.repeat.divideScalar(1.05);

                        mat.map.anisotropy = (this.renderer?.capabilities?.getMaxAnisotropy?.() ?? 8);

                        mat.map.needsUpdate = true;
                    });
                }

                if (x.name.startsWith('folds_6_1')) {
                    const mats = Array.isArray(x.material) ? x.material : [x.material];
