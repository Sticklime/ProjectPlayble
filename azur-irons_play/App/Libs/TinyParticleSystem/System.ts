...
export class System {
  ...
  private updateInstanceAttributes() {
    ...
    const colorAttr = this.instancedGeometry.attributes[
      "instanceColor"
    ] as InstancedBufferAttribute;
    ...
    for (let i = 0; i < this.particles.length; i++) {
      ...
      colorAttr.setXYZW(i, p.color.r, p.color.g, p.color.b, p.opacity);
    }
  ...
  private lerpColorArray(factor: number, array: Color[]): Color {
    ...
    const floorIndex = Math.floor(exactIndex);
    const ceilIndex = Math.min(floorIndex + 1, lastIndex);
    const fromColor = array[floorIndex] as Color;
    const toColor = array[ceilIndex] as Color;
    return new Color(
  ...
  public spawnBillboard(options: ISpawnOptions) {
    ...
    this.particles.push({
      ...
      color: options.colorOverTime[0] as Color,
