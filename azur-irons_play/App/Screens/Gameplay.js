...
App.Gameplay = new Screen({
    ...
    Hooks: {
        ...
        update() {
            if (!this._tutorialCheckLast || performance.now() - this._tutorialCheckLast > 500) {
                this._tutorialCheckLast = performance.now();

                if (!this.tutorialRunShown) {
                    const now = performance.now();
                    if (now - this.lastActivityTime > 5000) {
                        this.showTutorialRun();
                        this.tutorialRunShown = true;
                    }
                }
            }
        },
    ...
    buildParticlesEffect() {
        ...
        for (const fragment of fragments) {
            ...
            for (let i = 0; i < particleCount; i++) {
                ...
                const radius = 0.16 + Math.random() * 0.19;
                const startX = sprite.position.x + Math.cos(angle) * radius;
                const startZ = sprite.position.z + Math.sin(angle) * radius;
                const startY = sprite.position.y + 0.18 + Math.random() * 0.10;

                const scale = Math.random() * 0.013 + 0.014;
                ...
                const animateParticle = () => {
                    ...
                    particle.material.opacity = 0.6 + Math.random() * 0.25;

                    const deltaX = (Math.random() - 0.5) * 0.23;
                    const deltaZ = (Math.random() - 0.5) * 0.23;

                    gsap.to(particle.position, {
                        x: startX + deltaX,
                        y: startY + 0.30 + Math.random() * 0.28,
                        z: startZ + deltaZ,
                        duration: 0.8 + Math.random() * 0.85,
                        ease: "sine.in",
                    });
                    gsap.to(particle.material, {
                        opacity: 0,
                        duration: 0.38 + Math.random() * 0.35,
                        delay: 0.82 + Math.random() * 0.67,
    ...
    buildCamera() {
        this.camera.position.set(0, 3.54, 7.8);
        this.camera.rotation.set(THREE.MathUtils.degToRad(-20.8), 0, 0);
        this.camera.fov = 45;
    ...
    buildHero: function () {
        ...
        console.log(asset.animations.map(clip => clip.name));

        const allClip = this.findClip(asset.animations, "A_All");
        ...
        const winClip = THREE.AnimationUtils.subclip(allClip, "A_Win", 412, 450);
        ...
        hero.collectObstacles(this.scene);
    ...
    buildEnemies() {
        ...
        for (let i = 0; i < anchors.length; i++) {
            ...
            const materialOrange = new THREE.MeshStandardMaterial({
                color: 0xff9902,
                roughness: 0.2,       // Меньше roughness — больше отражает
                metalness: 0.1,       // Больше металличность
                emissive: 0xff9902,   // Добавь эмиссию того же цвета (или чуть светлее)
                emissiveIntensity: 0.18, // Усиль свечение
                side: THREE.DoubleSide
            });

            const selectedMaterial = i < halfCount ? materialRed : materialOrange;
    ...
    buildCoins() {
        ...
        new TriggerPack({
            ...
            triggerScale: new THREE.Vector3(2.5, 2.5, 2.5),
    ...
    async onCollectCoin(worldTransform, geometry, material, instanceId) {
        ...
        const instancedMesh = this.scene.getObjectByName("SM_Coin");
        if (instancedMesh && typeof instanceId === 'number') {
            const tmpMatrix = new THREE.Matrix4();
            tmpMatrix.makeScale(0, 0, 0);
            instancedMesh.setMatrixAt(instanceId, tmpMatrix);
            instancedMesh.instanceMatrix.needsUpdate = true;
        }

        const coinMaterial = material.clone();
        coinMaterial.depthWrite = false;
        coinMaterial.transparent = true;
        coinMaterial.opacity = 1;

        const coinMesh = new THREE.Mesh(geometry.clone(), coinMaterial);
        coinMesh.position.copy(worldTransform.position);
        coinMesh.position.y += 0.01;
        ...
        const magnetDuration = 0.12;
        const startTime = performance.now();

        const updateMagnet = (now) => {
            const t = Math.min(1, (now - startTime) / (magnetDuration * 1000));
            const heroPos = this.hero.position.clone();

            coinMesh.position.lerpVectors(worldTransform.position, heroPos.add(new THREE.Vector3(0, 0.4, 0)), t);

            if (t < 1 && this.scene.children.includes(coinMesh)) {
    ...
    showTutorialRun() {
        ...
        if (this.isShowTutorialRun)
            return;

        this.isShowTutorialRun = true;

        const handStartX = this.handStartX || hand.position.x;
        const handStartY = this.handStartY || hand.position.y;

        if (!this.handStartX) this.handStartX = handStartX;
        if (!this.handStartY) this.handStartY = handStartY;

        hand.position.x = this.handStartX;
        hand.position.y = this.handStartY;
        ...
        gsap.to(hand.position, {
            ...
            motionPath: {
                path: [
                    {x: hand.position.x, y: hand.position.y},
    ...
    hideTutorialRun() {
        ...
        this.isShowTutorialRun = false;
        this.tutorialRunShown = false;
    },
    ...
    showTutorialBuilder() {
        ...
        gsap.to(hand.position, {
            ...
            motionPath: {
                path: [
                    {x: -180, y: -290}, // Левое синее тело
                    {x: -100, y: -290}, // Левое красное тело (с пауком)
                    {x: 0, y: -290},    // Центральное красное тело
                    {x: 380, y: -290},  // Правое розовое тело
                    {x: 150, y: -500},   // Правое чёрное тело
                    {x: -180, y: -500},
                    {x: -180, y: -290},
                ],
                align: "self",
    ...
    async onSelectFragment(fragment) {
        ...
        switch (lastType) {
            ...
            case FragmentTypeName.legs: {
                ...
                this.playReadyEffect(new THREE.Vector3(0, 0.8, 0.4));
    ...
    runAnimation() {
        ...
        this.cameraOperator.distance = 1.5;
        ...
        const stopMovementIfArrived = async () => {
            ...
            if (currentPos.distanceTo(targetPosition) <= 0.5) {
                ...
                this.cameraOperator.isActive = true;
                this.cameraOperator.elevation = 45;
                this.cameraOperator.distance = 9.5;
                this.cameraOperator.speed = 8;
    ...
    async onChestProximity() {
        ...
        this.movementController.isActive = false;
        heroObject.playWin();
        this.showWinConfettiUI();

        this.tutorialRunShown = true;

        if (window.MraidSDK) MraidSDK.playSound("S_Win", {loop: true});
        else this.playSound("S_Win", {loop: true});

        const circle = this["Circle_small"];
        const tapToReward = this["Tap_To_Take"];
        const handOnCircle = this["T_Hand_On_Circle_Small"];

        circle.visible = true;
        tapToReward.visible = true;
        handOnCircle.visible = true;

        let lastTap = 0;
        const doubleTapThreshold = 300; // ms

        const onMobileTouch = () => {
            console.log("TOUCH on mobile!");
        ...
        document.addEventListener("touchstart", onMobileTouch, {once: true});

        TimeHandler.instance.on(
            TimeHandler.Event.tick,
            () => this.tutorialRunShown = true);

        gsap.fromTo(circle.scale,
            {x: 1, y: 1},
            {
                x: 0.7, y: 0.7,
                duration: 0.6,
    ...
    showWinConfettiUI(count = 120, durationTotal = 3.5) { // durationTotal — общее время "засыпания"
        const confettiTextures = [
            App.ThreeAssets["confetti_1"],
            App.ThreeAssets["confetti_2"],
            App.ThreeAssets["confetti_3"],
            App.ThreeAssets["confetti_4"],
            App.ThreeAssets["confetti_5"],
            App.ThreeAssets["confetti_6"],
            App.ThreeAssets["confetti_7"],
            App.ThreeAssets["confetti_8"],
            App.ThreeAssets["confetti_9"]
        ].map(asset => asset.texture ? asset.texture : asset);
        ...
        for (let i = 0; i < count; i++) {
            const appearDelay = Math.random() * durationTotal * 1000; // ms
            setTimeout(() => {
                const texture = confettiTextures[Math.floor(Math.random() * confettiTextures.length)];
                const material = new THREE.SpriteMaterial({map: texture, transparent: true, opacity: 1});
                const confetti = new THREE.Sprite(material);

                const startX = (Math.random() - 0.5) * uiWidth;
                const startY = uiHeight / 2 + 500 + Math.random() * 250; // выше!
                const startZ = 0.2 + Math.random() * 0.2;

                confetti.position.set(startX, startY, startZ);

                const size = 60 + Math.random() * 80;
                confetti.scale.set(size, size, 1);
                confetti.material.rotation = Math.random() * Math.PI * 2;

                ui.add(confetti);

                const targetX = startX + (Math.random() - 0.5) * 280;
                const targetY = -uiHeight / 2 - 120 - Math.random() * 120;
                ...
                gsap.to(confetti.position, {
                    ...
                    ease: "power1.in",
                    onComplete: function () {
                        if (confetti.parent) confetti.parent.remove(confetti);
                        material.dispose();
                    }
                });

                gsap.to(confetti.material, {
                    opacity: 0,
