import { EmbeddedDRACOLoader } from 'EmbeddedDRACOLoader';
...
Class.mixin(Game, {
	...
	*/
				...
				this.each(assets, function(asset) {
					...
					loaders.push(new Promise(resolve => {
						...
						App.ThreeLoaders[asset.name] = three_loader.load(url, object => {
						  object.colorSpace = THREE[asset.colorSpace ?? "SRGBColorSpace"];
	...
	loadThreeModel(assets) {
		...
		return new Promise(resolve => {
			...
			function loadModelOrderGroup(group) {
				...
				const loaders = group.map(async (asset) => {
					...
					if (asset.type === 'fbx') three_loader = new FBXLoader(manager).setCrossOrigin('*');
					else if (asset.type === 'glb') {
					  three_loader = new GLTFLoader(manager).setCrossOrigin('*')
						three_loader.setDRACOLoader(new EmbeddedDRACOLoader());
					};
    ...
    if (this.World.Renderer) {
      if (Array.isArray(this.layers) && this.layers.length > 0) {
        ...
        for (const layer of this.layers) {
          layer.render(this.World.Renderer, TimeController.instance.deltaTime);
        }
      } else if (!this.World.ThreeGUI) {
        this.World.Renderer.render(this.World.Scene, this.World.Camera);
      } else {
        this.World.Renderer.clear();
        this.World.Renderer.render(this.World.Scene, this.World.Camera);
	...
	resizeThree() {
		...
		if (this.World.Renderer) this.resizeThreeRenderer(this.Width, this.Height, this._screenWidth, this._screenHeight, this._screenLeft, this._screenTop);
	...
	resizeThreeRenderer(resolution_width, resolution_height, screen_width, screen_height, screen_left, screen_top) {
	  ...
	  const aspect = resolution_width / resolution_height;

		if (this.World.camera.type === 'OrthographicCamera') {
		  const verticalView = this.World.Camera.top - this.World.Camera.bottom;
			const horizontalView = verticalView * aspect;

			this.World.Camera.left = -horizontalView / 2;
			this.World.Camera.right = horizontalView / 2;
	...
	parseThreeSpritesheet(asset, callback) {
		...
		const metaScale = asset.json.meta.scale;
		const scale = metaScale !== undefined ? parseFloat(metaScale) : 1;
		let _batchIndex = 0;

		function extractSpriteTexture(originalTexture, meta, spriteData) {
    const texture = originalTexture.clone();
    texture.needsUpdate = true;

    const atlasWidth = meta.size.w;
    const atlasHeight = meta.size.h;

    const frame = spriteData.frame;
    const rotated = spriteData.rotated;

    const u = frame.x / atlasWidth;
    const v = 1 - (frame.y + frame.h) / atlasHeight; // Инвертируем Y
    const uScale = frame.w / atlasWidth;
    const vScale = frame.h / atlasHeight;
    ...
    if (rotated) {
        ...
        texture.repeat.set(vScale, uScale);
    }
    ...
    return texture;
		...
		const _processFrames = (initialFrameIndex) => {
			...
			while (frameIndex - initialFrameIndex < BATCH_SIZE && frameIndex < _frameKeys.length) {
				...
				const spriteData = _frames[i];
				const meta = asset.json.meta;
      const texture = extractSpriteTexture(baseTexture, meta, spriteData);
