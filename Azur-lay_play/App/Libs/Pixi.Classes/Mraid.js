...
let MRAID = {
  ...
  start() {
    if (window["MraidSDK"]) {
      Broadcast.on(
        "End Screen Showed",
        () => {
          if (window.MraidSDK && App.CallToAction["try again"])
            App.CallToAction["try again"].visible =
              MraidSDK.isReplayAvailable();
        },
        this,
      );
        ...
        () => {
          ...
          if (Settings["publisher"] === "rovio") {
            ...
            if (App.Engine === "Three" && App.World) {
              if (App.World.Renderer && App.World.Renderer.domElement)
                div.appendChild(App.World.Renderer.domElement);
            }
          }
        },
        this,
    ...
    if (!new_asset && old_asset) {
      ...
      if (new_asset.type === "image" || new_asset.type === "atlas") {
        ...
        App.loadAssets(
          [new_sprite_name],
          () => {
            App.resources[old_asset_name] = PIXI.utils.TextureCache[
              old_asset_name
            ] = PIXI.utils.TextureCache[new_sprite_name];
            ...
            _.each(App.Screens, (screen) => {
              _.each(screen._childs, (child) => {
                if (
                  child &&
                  child.params &&
                  child.params.image === old_asset_name
                ) {
                  if (child.params.type === "gif") {
                    let buffer = PIXI.gif.AnimatedGIF.fromBuffer(
                      App.resources[new_sprite_name].data,
                    );
        ...
        let asset = (App.Assets[old_asset_name] = Object.assign(
          App.Assets[old_asset_name] || { name: old_asset_name },
          new_asset,
        ));

        if (
          asset.url.indexOf("http") !== 0 &&
          asset.url.indexOf("data:") !== 0 &&
          asset.url.indexOf("file:") !== 0
        )
  ...
  processDynamicProperties(object, options = null) {
    ...
    if (options && options.orientation)
      orientation = options.orientation.toLowerCase();

    const isSquare = size
      ? size.width / size.height <= 1.1 && size.width / size.height >= 0.9
      : false;
    ...
    function is_postfix_exist(property_name, property_postfix) {
      return (
        property_name.indexOf(property_postfix) > -1 ||
        property_name.indexOf(":" + property_postfix.toLowerCase()) > -1
      );
    }

    function remove_postfix(property_name, property_postfix) {
      return property_name
        .replace(property_postfix, "")
        .replace(":" + property_postfix.toLowerCase(), "");
    }
      ...
      if (object.hasOwnProperty(property_name)) {
        ...
        for (let i = 0; postfixes[i]; i++) {
          if (is_postfix_exist(base_property_name, postfixes[i].name)) {
            ...
            base_property_name = remove_postfix(
              base_property_name,
              postfixes[i].name,
            );
    ...
    for (let property_name in dynamic_properties)
      if (dynamic_properties.hasOwnProperty(property_name)) {
        // Сохраним значение свойства которое было до первой замены (даже если там undefined)
        if (!(property_name + "#original" in object))
          object[property_name + "#original"] = object[property_name];
    ...
    for (let property_name in object)
      if (object.hasOwnProperty(property_name)) {
        if (
          property_name.indexOf("#original") > -1 &&
          !(property_name.replace("#original", "") in dynamic_properties)
        )
          object[property_name.replace("#original", "")] =
    ...
    for (let key in styles)
      if (styles.hasOwnProperty(key)) {
        ...
        if (styles[key].indexOf("url(") !== -1) {
          if (
            styles[key].indexOf("http") === -1 &&
            styles[key].indexOf("//") === -1 &&
            styles[key].indexOf("data:") === -1
          ) {
            styles[key] = styles[key].replace(
              "url(",
              "url(" + (Settings["assets-path"] || ""),
            );
          }
  ...
  isAutoplaySupported(callback) {
    window["MraidSDK"]
      ? MraidSDK.isAutoplaySupported(callback)
      : callback(true);
  },
