...
let Game = new Class({
  ...
  async start() {
    ...
    this.Loader = this.loadAssets(
      ...
      () => {
        ...
        setTimeout(() => {
          this.DelayedLoader = this.loadAssets(
            null,
            () => {
              if (window.MraidSDK) MraidSDK.track("Delayed Assets Loaded");
              else Broadcast.call("Delayed Assets Loaded");
            },
            { strategy: "delayed" },
          );
        }, 200);
        ...
        if (assets && name) {
          ...
          _.each(assets, (asset) => {
            if (
              ...
              loader.states[name] = "loading";
          });
        }

        let result = true;

        _.each(loader.states, (m) => {
          if (m !== "ready") result = false;
        });
  ...
  extractAssetsForLoad(loader, assets, next, options) {
    _.each(Settings.Assets, (asset, name) => {
      ...
      if (
        !asset.url &&
        _.includes(["image", "atlas", "sound", "font"], asset.type)
      )
        return;
      ...
      if (asset.type === "sound") {
        if (!asset.loadStrategy) {
          if (Settings["sounds"] === true) asset.loadStrategy = "preload";
            ...
            Settings["sounds"] === "delayed" ||
            typeof Settings["sounds"] === "undefined"
          )
            asset.loadStrategy = "delayed";
        }
      }
      ...
      if (this.Assets[name] && this.Assets[name].state !== "prepared") {
        if (this.Assets[name].state === "loaded")
          console.warn(
            'Asset already loaded. Check "' +
              name +
              '" asset for multiple load.',
          );
        else if (this.Assets[name].state === "loading")
          console.warn(
            'Asset already loading. Check "' +
              name +
              '" asset for multiple load.',
          );
        else
          throw new Error(
            'Asset names in all screens must be unique. Check "' +
              name +
              '" asset definition in "' +
              screen.Name +
              '" screen.',
      ...
      if (asset.url) {
        ...
        if (asset.url.indexOf("http") !== 0 && asset.url.indexOf("data:") !== 0)
          asset.url = (Settings["assets-path"] || "") + asset.url;
    ...
    if (Settings["video"]) {
      let asset = (App.Assets["#start-video"] = {
        name: "#start-video",
        type: "video",
        url: Settings["video"],
        loadTillCanPlay: true,
        state: "prepared",
      });
      ...
      _.each(assets, (asset) => {
        asset.sound = new Howl({
          ...
          onload: () => {
            ...
            _.each(assets, (asset) => {
              if (
                !App.Assets[asset.name] ||
                App.Assets[asset.name].state !== "loaded"
              )
                loader.states["sound"] = "loading";
            });
  ...
  loadTexts(loader, assets) {
    if (assets && assets.length > 0) {
      _.each(assets, (asset) => {
        if (!asset.json && asset.pako)
          asset.json = JSON.parse(pako.inflate(asset.pako, { to: "string" }));
        ...
        if (asset["jsmpeg"]) {
          ...
          if (asset["jsmpeg-preload"]) {
            ...
            asset.LoadedInterval = setInterval(() => {
              MRAID.log(
                "jsmpeg video loaded check",
                "needed: " + asset["jsmpeg-preload"] * 100 + "%",
                "loaded: " +
                  Math.round(
                    (asset.video.source.loadedSize /
                      asset.video.source.fileSize) *
                      100,
                  ) +
                  "%",
                asset.video.source.loadedSize,
              ...
              if (
                asset["jsmpeg-preload"] <=
                asset.video.source.loadedSize / asset.video.source.fileSize
              ) {
          ...
          asset.video = new JSMpeg.Player(url.replace(".mp4", ".ts"), {
            loop: false,
            autoplay: false,
            ...
            onSourceEstablished: () => {
              MRAID.log(
                "jsmpeg video onSourceEstablished",
                Math.floor(Date.now() / 1000),
              );
            },
            onSourceCompleted: () => {
              MRAID.log(
                "jsmpeg video onSourceCompleted",
                Math.floor(Date.now() / 1000),
              );
              ...
              setTimeout(() => {
                ...
                asset.video.duration =
                  asset.video.video.timestamps.length /
                  asset.video.video.frameRate;
              }, 0);
          ...
          if (asset["loadTillCanPlay"] || asset["loadAsVideoTag"]) {
            asset.video.addEventListener(
              MRAID.ios ? "loadedmetadata" : "canplay",
              function video_can_play() {
                asset.video.removeEventListener(
                  MRAID.ios ? "loadedmetadata" : "canplay",
                  video_can_play,
                );
                ...
                if (!asset["loadTillCanPlay"]) {
                  ...
                  MRAID.log(
                    "mark assets video as loaded after loaded metadata (loadAsVideoTag)",
                    asset.name,
                  );
                  ...
                  function video_time_update() {
                    ...
                    if (asset.video.currentTime > 0.1) {
                      ...
                      if (asset["loadTillCanPlay"]) {
                        ...
                        MRAID.log(
                          "mark assets video as loaded after timeupdate (loadTillCanPlay)",
                          asset.name,
                        );
                ...
                if (asset["forceLoadOnEveryClick"]) {
                  document.addEventListener(
                    App.IsTouchDevice ? "touchstart" : "mousedown",
                    function video_first_click() {
                      MRAID.log(
                        "load video interaction",
                        asset.video.userPlayOccured,
                        asset.video.currentTime,
                      );
                      ...
                      if (!asset.video.userPlayOccured) {
                        ...
                        MRAID.log(
                          "load video interaction play() to force preload",
                        );
                      } else {
        ...
        for (let i = 0; assets[i]; i++) {
          ...
          if (asset.state !== "loading" && asset.state !== "loaded") {
            if (asset["jsmpeg"]) {
              ...
              asset["loadTillCanPlay"] === true ||
              asset["loadAsVideoTag"]
            ) {
              asset.video.src = asset.url;
              asset.video.load();
  ...
  update(time, deltaTime, frame) {
    ...
    if (!this._viewportSizeProtectionCount)
      this._viewportSizeProtectionCount = 0;

    if (
      Settings["viewport-size-protection"] &&
      this._viewportSizeProtectionCount < 5
    ) {
      if (
        !this._viewportSizeProtectionTime ||
        this.time - this._viewportSizeProtectionTime > 2000
      ) {
        ...
        let new_size = (this._vewport_size = window.MraidSDK
          ? MraidSDK.getSize()
    ...
    if (asset && asset.sound) {
      ...
      try {
        ...
        if ("volume" in options)
          asset.sound.volume(options.volume, soundInstanceId);
