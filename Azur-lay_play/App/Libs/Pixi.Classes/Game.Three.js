...
Class.mixin(Game, {
    ...
    return new Promise((resolve) => {
      ...
      if (assets && assets.length > 0) {
        ...
        this.each(assets, function (asset) {
          if (!window.THREE)
            throw new Error(`Connect three.js to load "${asset.name}" asset`);
          ...
          loaders.push(
            new Promise((resolve) => {
              App.ThreeLoaders[asset.name] = three_loader.load(
                url,
                (object) => {
          ...
          loaders.push(
            new Promise((resolve) => {
              ...
              if (asset.json) {
                ...
                App.ThreeAssets[asset.name].json = asset.json;
                ...
                loaders.push(
                  new Promise((resolve) => {
                    App.ThreeLoaders[asset.name + " image"] = three_loader.load(
                      url,
                      (image) => {
                        App.ThreeAssets[asset.name].image = image;

                        this.parseThreeSpritesheet(
                          App.ThreeAssets[asset.name],
                          () => {
                            resolve();
                          },
                ...
                App.ThreeLoaders[asset.name + " json"] = three_json_loader.load(
                  url,
                  (object) => {
                    if (object.meta) {
                      App.ThreeAssets[asset.name] = {};
                      ...
                      url = url.replace(/[^/]+$/gi, object.meta.image);
                      ...
                      loaders.push(
                        new Promise((resolve) => {
                          App.ThreeLoaders[asset.name + " image"] =
                            three_loader.load(
                              url,
                              (image) => {
                                App.ThreeAssets[asset.name].image = image;
  ...
  loadThreeModel(assets) {
    return new Promise((resolve) => {
      ...
      assets.forEach(
        (asset) =>
          (asset.order =
            typeof asset.order === "number" ? asset.order : Infinity),
      );
        ...
        const loaders = group.map((asset) => {
          ...
          manager.setURLModifier((url) => {
            if (
              ...
              if (!App.Assets[name])
                throw new Error(
                  `FBX file require "${name}" source. Add this asset to settings Assets.`,
                );
          ...
          if (asset.type === "fbx")
            three_loader = new FBXLoader(manager).setCrossOrigin("*");
          else if (asset.type === "glb")
            three_loader = new GLTFLoader(manager).setCrossOrigin("*");

          let url = asset.pako
            ? pako.inflate(asset.pako, { to: "string" })
            : asset.url;
  ...
  createThree() {
    if (Settings["force-canvas-renderer"])
      throw new Error(
        "Enable WebGL to load three.js (make 'force-canvas-renderer' = false)",
      );
  ...
  createThreeWorld: function (config) {
    ...
    if (config.Renderer.domElement.parent)
      config.Renderer.domElement.parent.removeChild(config.Renderer.domElement);
    ...
    if (config.background)
      config.Scene.background = new THREE.Color(config.background);
    ...
    if (window.ThreeGUI) {
      const optionsThreeGUI = {
        sizeLandscape:
          config.threeGUI && config.threeGUI.sizeLandscape
            ? config.threeGUI.sizeLandscape
            : [1920, 1080],
        sizePortrait:
          config.threeGUI && config.threeGUI.sizePortrait
            ? config.threeGUI.sizePortrait
            : [1080, 1920],
      };

      this.World.ThreeGUI = new ThreeGUI(this, optionsThreeGUI);
    }
        ...
        Broadcast.on(
          ...
          (e, position) => {
            ...
            if (this.World.ThreeGUI) {
              if (this.World.ThreeGUI.eventHandler(event_type, e, position))
                return;
            }

            position = {
              x: position.x - this.World.Width / this.PixelRatio / 2,
              y: position.y - this.World.Height / this.PixelRatio / 2,
            };
            ...
            let intersects = this.ThreeRaycaster.intersectObjects(
              this.World.EventsObjects,
              true,
            );
            ...
            if (intersects[0]) {
              ...
              if (event)
                Broadcast.call(
                  obj._screen_name + " " + event + " " + event_type,
                  [obj, e],
                );
            }
          },
  ...
  startThreeTicker() {
    gsap.ticker.add((time, deltaTime, frame) => {
      this.update(time, deltaTime, frame);
    });
  },
  ...
  addThreeRendererEvents() {
    if (App.IsTouchDevice) {
      this.World.Renderer.domElement.addEventListener(
        "touchstart",
        function (e) {
          Broadcast.call("Stage Press Down", [
            e,
      ...
      this.World.Renderer.domElement.addEventListener(
        "touchmove",
        function (e) {
          Broadcast.call("Stage Press Move", [
            e,
            { x: e.changedTouches[0].clientX, y: e.changedTouches[0].clientY },
          ]);
  ...
  parseThreeSpritesheet(asset, callback) {
    ...
    const _processFrames = (initialFrameIndex) => {
      ...
      while (
        frameIndex - initialFrameIndex < BATCH_SIZE &&
        frameIndex < _frameKeys.length
      ) {
        ...
        if (rect) {
          ...
          texture.repeat.set(
            (frame.width / baseTexture.image.width) * resolution,
            (frame.height / baseTexture.image.height) * resolution,
          );
          texture.offset.x = (frame.x / baseTexture.image.width) * resolution;

          if (data.rotated) {
            texture.offset.y =
              1 - (frame.y / baseTexture.image.height) * resolution;
            texture.rotation = Math.PI / 2;
          } else {
            texture.offset.y =
              1 -
              frame.height / baseTexture.image.height -
              (frame.y / baseTexture.image.height) * resolution;
            texture.rotation = 0;
          }
