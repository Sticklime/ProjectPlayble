...
Screen.DOM = Class.mixin(Screen, {
	...
	resizeDOM() {
		...
		_.each(this._dom_containers, container => {
			...
			let size = window.MraidSDK ? MraidSDK.getSize() : {width: window.innerWidth, height: window.innerHeight};
	...
	getDOMScaleByStrategy(scale_strategy) {
		...
		const
			real_width = window.MraidSDK ? MraidSDK.getSize().width : window.innerWidth,
			real_height = window.MraidSDK ? MraidSDK.getSize().height : window.innerHeight;
